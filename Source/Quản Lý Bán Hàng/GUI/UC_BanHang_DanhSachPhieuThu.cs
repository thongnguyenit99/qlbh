...
{
    public partial class UC_BanHang_DanhSachPhieuThu : DevExpress.XtraEditors.XtraUserControl
    {
        UC_BanHang_DanhSachPhieuThuBUS uC_BanHang_DanhSachPhieuThuBUS = new UC_BanHang_DanhSachPhieuThuBUS();
        ...
        public UC_BanHang_DanhSachPhieuThu()
        {
            InitializeComponent();
            int tag1 = 26;
            ...
            grvdspt.Columns["TenKH"].Group();
            GridGroupSummaryItem item = new GridGroupSummaryItem();
            item.FieldName = "TongTien";
            //item.ShowInGroupColumnFooter = gridView.Columns["OrderSum"];
            item.SummaryType = DevExpress.Data.SummaryItemType.Sum;
            item.DisplayFormat = "- Số tiền = {0}";
            grvdspt.GroupSummary.Add(item);

            GridColumn column = grvdspt.Columns["TongTien"];
        ...
        {
            grvctdsthutien.DataSource = uC_BanHang_DanhSachPhieuThuBUS.LayDanhSachPhieuThuTheoNgay(txttungay.EditValue.ToString(), txtdenngay.EditValue.ToString());
        }
        ...
        private void btnxuat_Click(object sender, EventArgs e)
        {
            ...
            {
                grvdspt.ExportToXlsx(fbd.SelectedPath + "\\DanhSachPhieuThu" + i + ".xlsx");
                i++;
            }
        }
        ...
        private void btnxoa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string temp = grvdspt.GetRowCellValue(grvdspt.FocusedRowHandle, "MaPT").ToString();
            }
            catch
            {
                i = -1;
            ...
            if (i == 1)
            {
                if (uC_BanHang_DanhSachPhieuThuBUS.XoaPhieuThuTien(grvdspt.GetRowCellValue(grvdspt.FocusedRowHandle, "MaPT").ToString(), grvdspt.GetRowCellValue(grvdspt.FocusedRowHandle, "MaPBH").ToString()) < 0)
                {
                    MessageBox.Show("Xóa Phiếu Chi Thất Bại", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    LoadData();
        ...
        private void grvctdsthutien_DoubleClick(object sender, EventArgs e)
        {

        }

        private void grvdspt_DoubleClick(object sender, EventArgs e)
        {
           
        }

        private void grvctdsthutien_MouseDoubleClick(object sender, MouseEventArgs e)
        {
